 ...
 */
export const apiSecurityInterceptor = (request, next) => {
    if (!isChoomVaultApiRequest(request.url)) {
        return next(request);
    }
    const csrfService = inject(CsrfService);
    const authState = inject(AuthStateService);
    const send = (securedRequest) => next(securedRequest).pipe(catchError((error) => {
    ...
    return csrfService.getRequestToken().pipe(switchMap(requestToken => send(credentialedRequest.clone({
        setHeaders: {
