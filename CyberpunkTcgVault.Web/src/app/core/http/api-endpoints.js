...
export const API_ENDPOINTS = {
    ...
    cardById: (id) => apiUrl(`/api/Cards/${id}`),
    ownedCards: apiUrl('/api/OwnedCards'),
    ownedCardById: (id) => apiUrl(`/api/OwnedCards/${id}`),
    wishlist: apiUrl('/api/WishListItem'),
    wishlistItemById: (id) => apiUrl(`/api/WishListItem/${id}`),
    auth: {
 ...
 * antiforgery header.
 */
export const isChoomVaultApiRequest = (url) => url === apiBaseUrl || url.startsWith(`${apiBaseUrl}/`);
/**
