...
export class CapabilitiesService {
  private readonly capabilitiesState = signal<ProductCapabilities>(
  ...
  readonly demoAccessEnabled = computed(
    () => this.capabilitiesState().demoAccessEnabled
  );
  ...
  load(): Observable<ProductCapabilities> {
    ...
    this.loadRequest$ = this.http
      .get<ProductCapabilities>(API_ENDPOINTS.capabilities)
      .pipe(
        tap(capabilities => {
