...
describe('CardAnatomyShowcase', () => {
  ...
  it('uses the Vesper Ryne Crimson Echo prototype study card and preserves all 11 anatomy fields', () => {
    ...
    expect(image.getAttribute('src')).toBe(
      'images/showcase/vesper-ryne-crimson-echo.webp'
    );
    ...
    expect(
      fixture.componentInstance.anatomyFields.map(
        field => field.id
      )
    ).toEqual([
      'cost',
  ...
  it('uses the Vesper Ryne-specific lower-card alignment without changing the anatomy order', () => {
    ...
    expect(setCode.region.left)
      .toBeGreaterThan(cardNumber.region.left);

    expect(setCode.region.top)
      .toBeGreaterThan(cardNumber.region.top);

    expect(setCode.marker.left)
      .toBeGreaterThan(0);

    expect(cardNumber.marker.left)
      .toBeLessThan(0);

    expect(rulesText.region.width)
      .toBe(82.5);

    expect(rulesText.region.height)
      .toBe(21.2);
  });
  ...
  it('keeps all 11 Show All markers available for the Vesper Ryne study card', async () => {
    ...
    const showAllButton = modeButtons.find(
